var initialize_force_directed = function () {
    ...
    force.on("tick", function () {
        ...
        link
            .each(function (d) {
                d.startx = function () {
                    return this.source.x_list[modes.source_generation];
                };
                d.starty = function () {
                    return this.source.y_list[modes.source_generation];
                };
                d.endx = function () {
                    return this.target.x_list[modes.target_generation];
                };
                d.endy = function () {
                    return this.target.y_list[modes.target_generation];
                };
            });


        node.attr("cx", function (d) {
                d.x_list[modes.active_generation] = d.x;
                return d.x_list[modes.active_generation];
            })
            .attr("cy", function (d) {
                d.y_list[modes.active_generation] = d.y;
...
var set_xscale_by_quantitative_property = function (prop) {
    xscale = d3.scale.linear()
        .range([0, width])
        .domain([0, d3.max(node_data().map(function (d) {
            return d[prop];
        }))])
        .nice()
};
...
var substrate_on_y = function (prop) {
    ...
    yscale = d3.scale.ordinal()
        .domain(y_substrates.map(function (d) {
            return d.key;
        }))
...
var substrate_on_x = function (prop) {
    ...
    xscale = d3.scale.ordinal()
        .domain(x_substrates.map(function (d) {
            return d.key;
        }))
...
var scatter_on_x = function () {
    ...
    y_substrates.forEach(function (category) {
        category.xscale = d3.scale.ordinal()
            .domain(category.values.map(function (d) {
                return d.id
            }))
...
var update_rolled_up = function () {
    if (activeGenIsAggregate()) {
        ...
        node_generations[agg_generations[modes.active_generation].source_gen]
            .attr("cx", function (d) {
                return d.x_list[agg_generations[modes.active_generation].source_gen]
            })
            .attr("cy", function (d) {
                return d.y_list[agg_generations[modes.active_generation].source_gen]
            })
    }
};
