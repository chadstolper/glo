...
GLO.NodeGeneration.prototype.update = function(){
	...
	if(this.is_aggregated){
		for(var [n,list] of this.aggregate_node_map){
			for(var d in list){
				...
				d.hover_list.set(self.aggregate_source_generation.gen_id, n.hover_list.get(self.gen_id))
			}
		}
	...
	this.node_glyphs.transition()
		...
		.attr("stroke", function(d){
			if(d.hover_list.get(self.gen_id) == true){
				return "black"
			}else{
				return null
			}
...
GLO.NodeGeneration.prototype.init_props = function(){
	...
	this.nodes
		.forEach(function(d){
			...
			d.hover_list.set(self.gen_id, false)
		})
...
GLO.NodeGeneration.prototype.init_draw = function(){
	...
	this.node_glyphs
		...
		.on('mouseover', function(d){
			d.hover_list.set(self.gen_id, true)
			self.update()
		})
